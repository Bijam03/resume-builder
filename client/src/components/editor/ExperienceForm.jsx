...
const ExperienceForm = () => {
  ...
  const addItem  = () => updateSection('experience', [...list, { ...empty }]);
  const removeItem = (index) =>
    updateSection('experience', list.filter((_, i) => i !== index));

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-gray-700">Experience</h2>
        <button
          onClick={addItem}
      ...
      {list.map((item, index) => (
        <div key={index} className="border border-gray-200 rounded-xl p-4 space-y-3">
          ...
          {[
            { label: 'Company',   name: 'company',   placeholder: 'Google' },
            { label: 'Role',      name: 'role',      placeholder: 'Frontend Developer' },
            { label: 'Start Date',name: 'startDate', placeholder: 'Jan 2022' },
